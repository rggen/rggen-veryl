...
RSpec.describe 'bit_field/type/rwtrg' do
  ...
  it '出力ポート#value_out/#write_trigger/#raed_triggerを持つ' do
    ...
    expect(bit_fields[2]).to have_port(
      :register_block, :write_trigger,
      name: 'o_register_0_bit_field_2_write_trigger', direction: :output, type: :logic, width: 1,
      array_size: [2]
    )
    expect(bit_fields[2]).to have_port(
      :register_block, :read_trigger,
      name: 'o_register_0_bit_field_2_read_trigger', direction: :output, type: :logic, width: 1,
      array_size: [2]
    )
